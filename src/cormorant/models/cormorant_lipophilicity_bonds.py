...
from cormorant.nn import InputLinear, InputEdgeLinear
from cormorant.nn import OutputLinear, GetScalarsAtom
from cormorant.nn import NoLayer


class CormorantLipophilicity_Bonds(CGModule):
    """
    Basic Cormorant Network used to train Lipophilicity.
    ...
    def __init__(self, maxl, max_sh, num_cg_levels, num_channels, num_species,
        ...
        num_edge_scalars_in = 2
        num_scalars_out = num_channels[0]

        self.input_func_atom = InputLinear(num_scalars_in, num_scalars_out,
                                           device=self.device, dtype=self.dtype)
        self.input_func_edge = InputEdgeLinear(num_edge_scalars_in, num_scalars_out,
                                               device=self.device, dtype=self.dtype)
    ...
    def prepare_input(self, data):
        ...
        bonds = data['bonds'].to(device, dtype)
